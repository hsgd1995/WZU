...
function getManagerList(){
	var index = 1;
	$.ajax({
		url:'manager/getOrdinaryManager',
		method:'get',
		dataType:'json',
		data:{
		},
		success : function(data) {
			fillManagerSelect(data);
		}
	});
}
 ...
 * 填充管理员下拉列表
 */
function fillManagerSelect(data){
	var obj = document.getElementById("managerSelect");
	obj.add(new Option('请选择...', -1));
	for (var index = 0; index < data.length; index++) {
        //添加一个选项
        obj.add(new Option(data[index].username, data[index].id));
    }
}
/**
* 请求角色与管理员数据事件
...
function fillData(object) {
	...
	var $td4 = $('<td class="contentOmit content">' + object.manager.id + '</td>');
	var $td5 = $('<td><span class="contentOmit content" TITLE="'+object.manager.username+'">' + object.manager.username + '</span></td>');
...
function addData(){
	...
	var confirmMessage = '确认分配 该角色给用户吗？';
	if(confirm(confirmMessage)){
		add(roleId,managerId);
	}
}
 ...
 * @param formData
 */
function add(roleId,managerId){
	$.ajax({
		url:'roleManager/add',
		method:'post',
		data:{roleId:roleId,managerId:managerId},
		dataType:'json',
		success : function(data){
			if(data.status){
				alert(data.message);
