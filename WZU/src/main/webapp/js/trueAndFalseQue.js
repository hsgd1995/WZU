...
function saveQuestion() {
	$('.editInfo .form-group .btn-primary').unbind()
			.click(function() {
				var test = $("#answer option:selected").val();
						/*
						 * var formParams =
						...
						formData.append("answer",$("#answer option:selected").val());
...
function clearQuestionFormFunc() {
	...
	$('#questionForm select[name=answer]').val('0');
...
function requestQuestion(index, keyword) {
	...
	$.ajax({
		url : 'jp/' + index + '/getQuestionPageModel',
		method : 'post',
		data : {
			"subject" : subject,
			"title" : title
...
function loadEditInfo(data) {
	...
	$('#questionForm select[name=answer]').val(data.answer);
