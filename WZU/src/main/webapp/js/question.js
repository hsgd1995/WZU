								...
								.ajax({
									...
									success : function(data) {
										...
										if (!data.status) {
											...
											if (data.failIndex) {
												noExist = noExist
														+ '<p style="color:red"><br>导入停止，存在不合格的记录，第'
														+ data.failIndex
														+ '条行记录存在空的单元格，该行记录之前的记录已导入</p>';
											}
...
function saveQuestion() {
	$('.editInfo .form-group .btn-primary').unbind().click(function() {
		/*
		 * var formParams = getParams($('#questionForm').serializeArray());
		 */
		// 校验数据
		var title = $("#questionForm [name=title]").val();
		var typeChoice = $("#typeChoice option:selected").val();
		var optionOne = $("#questionForm [name=optionOne]").val();
		var optionTwo = $("#questionForm [name=optionTwo]").val();
		var optionThree = $("#questionForm [name=optionThree]").val();
		var optionFour = $("#questionForm [name=optionFour]").val();
		var answer = new String($("#questionForm [name=answer]").val());
		...
		if (typeChoice == '0') {
			if (answer.length == '1') {
				if (answer.charAt(0) > '4' || answer.charAt(0) < '1') {
					alert('请输入正确格式答案！');
					return false;
				}
			}else{
			...
			for (var i = 0; i < answer.length; i++) {
				if (i % 2 == 0) {
					if (answer.charAt(i) > '4' || answer.charAt(i) < '1') {
						alert('请输入正确格式答案！');
						return false;
					}
				} else {
		...
		formData.append("typeChoice", $("#typeChoice option:selected").val());
...
function clearQuestionFormFunc() {
	...
	$('#questionForm select[name=typeChoice]').val('0');
	$('#questionForm input[name=optionOne]').val('');
	$('#questionForm input[name=optionTwo]').val('');
	$('#questionForm input[name=optionThree]').val('');
	$('#questionForm input[name=optionFour]').val('');
	$('#questionForm input[name=answer]').val('');
...
function requestQuestion(index) {
	...
	$.ajax({
		url : 'cq/' + index + '/getQuestionPageModel',
		method : 'post',
		data : {
			'subject' : subject,
			'title' : title
...
function fillQuestionData(pageModel) {
	...
	$.each($questionDate, function(index, question) {
		if (question.typeChoice == 0) {
			question.typeChoice = "单选题";
		} else {
			question.typeChoice = "多选题";
		}
...
function fillQuestion(question) {
	...
	var $td4 = $('<td>' + question.typeChoice + '</td>');
...
function loadEditInfo(data) {
	...
	$('#questionForm select[name=typeChoice]').val(data.typeChoice);
	$('#questionForm input[name=optionOne]').val(data.optionOne);
	$('#questionForm input[name=optionTwo]').val(data.optionTwo);
	$('#questionForm input[name=optionThree]').val(data.optionThree);
	$('#questionForm input[name=optionFour]').val(data.optionFour);
...
function onclickPage(pageModel) {
	...
	$('#pagination ul li:eq(1) a').attr("onClick",
			"pageQuestionSeach(" + $preIndex + ")");
	$('#pagination ul li:eq(3) a').attr("onClick",
			"pageQuestionSeach(" + $nextIndex + ")");
	$('#pagination ul li:eq(4) a').attr("onClick",
			"pageQuestionSeach(" + $totalSize + ")");
}